...
// Input validation schema for products
const productValidationSchema = celebrate({
  body: Joi.object({
    name: Joi.string().required().min(3).max(50),
    price: Joi.number().required().min(0),
...
app.post('/products', productValidationSchema, async (req, res) => {
  try {
    ...
    handleError(err, res, 'Error creating product');
  }
});
...
app.get('/products', async (req, res) => {
  try {
    ...
    handleError(err, res, 'Error retrieving products');
  }
});
...
app.put('/products/:id', productValidationSchema, async (req, res) => {
  try {
    ...
    handleError(err, res, 'Error updating product');
  }
});
...
app.delete('/products/:id', async (req, res) => {
  try {
    ...
    handleError(err, res, 'Error deleting product');
  }
});

// Error handler helper
function handleError(err, res, customMessage) {
  if (err.name === 'ValidationError') {
    const validationErrors = Object.values(err.errors).map(e => ({
      field: e.path,
      message: e.message
  ...
  res.status(500).json({ success: false, error: customMessage });
}
